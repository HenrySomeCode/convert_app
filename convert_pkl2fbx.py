...
class ConvertPkl2Fbx:
    ...
    def export_animated_mesh(self, output_path, gender):
        ...
        if output_path.endswith('.glb'):
            ...
            if list(finish)[0] == 'FINISHED':
                ...
                self.adding_pose_bar.progress(
                    self.adding_pose_bar_value, text=self.adding_pose_text)
            print('Exporting to FBX binary time : ',
    ...
    def run(self):
        ...
        self.export_animated_mesh(self.output_path, gender=self.gender)
    ...
    def convert(self, 
        ...
        def convert_a_file(input_path, output_path):
            
            print("converting a file >>>")
            import bpy
        ...
        def convert_a_folder(input_path, output_path):
            output_paths = []
            for pkl_f_name in os.listdir(input_path):
                pkl_f_path = os.path.join(input_path,pkl_f_name)
                # print(pkl_f_path)
                input_pkl_path = pkl_f_path
                output_fbx_path = os.path.join(output_path,pkl_f_name.replace('pkl','fbx'))
                convert_a_file(input_pkl_path,output_fbx_path)
                output_paths.append(output_fbx_path)

            print("zipping files ...")

            zip_f_path = os.path.join(output_path,"results.zip")
            ...
            for imgs in output_paths:
                os.remove(imgs)
            
        if bpy.app.background:

            if(os.path.isfile(input_path)):
                if input_path.lower().endswith(('pk','pkl','pickle')):
                    convert_a_file(input_path,output_path)

                elif input_path.endswith(('zip')):
                    os.makedirs("unzipped_pkl_files")

                    with zipfile.ZipFile(input_path, 'r') as zipf:
                        zipf.extractall("unzipped_pkl_files")

                    convert_a_folder(f"unzipped_pkl_files/{os.path.basename(input_path).split('.')[0]}", output_path)

                    shutil.rmtree("unzipped_pkl_files")

            elif (os.path.isdir(input_path)):
                convert_a_folder(input_path,output_path)
        
if __name__ == '__main__':

    pkl2fbx_converter = ConvertPkl2Fbx()
        
    # fps_source = 24
    # fps_target = 24
    # gender = "neutral"

    input_path = 'Pkls/final_all_3.pkl'
    output_path = 'output_fbxs/result3.fbx'
    
    pkl2fbx_converter.convert(input_path, output_path)

    # second run

    # input_path = 'Pkls/final_all_4.pkl'
    # output_path = 'output_fbxs/result4.fbx'

    # pkl2fbx_converter.convert(input_path, output_path)

    # a folder of pkl files
    input_path = 'Pkls'
