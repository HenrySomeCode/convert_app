...
male_model_path = '/mnt/datadrive/annh/ROMP/simple_romp/model_data/SMPL_unity_v.1.0.0/smpl/Models/SMPL_m_unityDoubleBlends_lbs_10_scale5_207_v1.0.0.fbx'
female_model_path = '/mnt/datadrive/annh/ROMP/simple_romp/model_data/SMPL_unity_v.1.0.0/smpl/Models/SMPL_f_unityDoubleBlends_lbs_10_scale5_207_v1.0.0.fbx'
...
def process_pose(current_frame, pose, trans, pelvis_position):
    ...
    bones[bone_name_from_index[0]].keyframe_insert(
...
def process_poses(
    ...
    sequence_results = np.load(input_path, allow_pickle=True)[
        'sequence_results'][()]
    ...
    if len(sequence_results) > 0:
        ...
        poses, trans = np.zeros((len(frame_names), 72)
                                ), np.zeros((len(frame_names), 3))
    ...
    while source_index < poses.shape[0]:
        ...
        process_pose(frame, poses[source_index],
                     (trans[source_index] - offset), pelvis_position)
